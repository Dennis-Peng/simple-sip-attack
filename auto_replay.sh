...
ATTACK_REPLAY_LOOP=3
ATTACK_REPLAY_SPEED=10
ATTACK_REPLAY_BULK=true
ATTACK_PCAPS=()
DUMP_SNAPLEN=0
DUMP_PCAP=./dump_output_$(date +'%Y%m%d%H%M%S').pcap
...
function main() {
    ...
    if [ -n "$DUMP_PCAP" ]; then
        ...
        # tcpdump -i $OFACE -w $DUMP_PCAP -s $DUMP_SNAPLEN -vv > "${DUMP_PCAP}.log" &
        tcpdump -i $OFACE -s $DUMP_SNAPLEN -w - | tee ${DUMP_PCAP} | tcpdump -v -r - > "${DUMP_PCAP}.log" &
        BG_DUMP_PID=$!
    fi
